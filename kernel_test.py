...
shape_post = np.asarray([((x_res - shape_kernel[0]) / pre_sample_steps[0]), ((y_res - shape_kernel[1]) / pre_sample_steps[1])])
weight_kernel = []
for i in range(int(shape_kernel[0])):
    weight_row = []
    for j in range(int(shape_kernel[1])):
        x_value = ((shape_kernel[0] / 2.) - np.abs(i - (shape_kernel[0] / 2.))) / (shape_kernel[0] / 2.)
        y_value = ((shape_kernel[1] / 2.) - np.abs(j - (shape_kernel[1] / 2.))) / (shape_kernel[0] / 2.)
...
spike_times = [[i, i] for i in range(int(x_res*y_res))]
